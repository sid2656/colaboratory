...
class Script(scripts.Script):
    def title(self):
        return "(Beta-self) Multi-frame Video rendering - V0.72"
    ...
    def ui(self, is_img2img):   
        ...
        input_dir = gr.Textbox(label='（二选一）Input directory', lines=1)
        reference_imgs = gr.UploadButton(label="（二选一）Upload Guide Frames", file_types = ['.png','.jpg','.jpeg'], live=True, file_count = "multiple") 
        ...
        return [input_dir,append_interrogation,reference_imgs, first_denoise, third_frame_image, color_correction_enabled, unfreeze_seed, loopback_source]

    def run(self, p, input_dir,append_interrogation, reference_imgs, first_denoise, third_frame_image, color_correction_enabled, unfreeze_seed, loopback_source):
        freeze_seed = not unfreeze_seed
        
        loops = len(input_dir)
        if(loops>0):
            reference_input_imgs = [
            os.path.join(
                input_dir,
                f) for f in os.listdir(input_dir) if re.match(
                r'.+\.(jpg|png)$',
                f)]
            loops = len(reference_input_imgs)
            if(loops>0):
                reference_imgs = reference_input_imgs
